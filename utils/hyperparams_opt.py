...
def hyperparam_optimization(algo, model_fn, env_fn, n_trials=10, n_timesteps=5000, hyperparams=None,
    ...
    def objective(trial):
        ...
        # if algo in ['her', 'aimsac']:
        if not isinstance(eval_env, HERGoalEnvWrapper):
...
def sample_sac_params(trial):
    ...
    tau = trial.suggest_categorical('tau', [0.1, 0.05, 0.01, 0.005])
    learning_rate = trial.suggest_loguniform('lr', 1e-5, 1)
    batch_size = trial.suggest_categorical('batch_size', [16, 32, 64, 128, 256, 512])
    buffer_size = trial.suggest_categorical('buffer_size', [int(3e5), int(1e6), int(1e7)])
    learning_starts = trial.suggest_categorical('learning_starts', [0, 1000, 10000, 20000])
    train_freq = trial.suggest_categorical('train_freq', [1, 10, 100])
    # gradient_steps takes too much time
    # gradient_steps = trial.suggest_categorical('gradient_steps', [1, 100, 300])
    gradient_steps = trial.suggest_categorical('gradient_steps', [1, 5, 10, 100])
...
def sample_her_params(trial):
    ...
    hyperparams['n_sampled_goal'] = trial.suggest_categorical('n_sampled_goal', [1, 4, 8])
