...
from stable_baselines.her import HERGoalEnvWrapper, SMiRLHERGoalEnvWrapper
...
if __name__ == '__main__':
    ...
    parser.add_argument('--smirl', action='store_true', default=False,
                        help='whether to use smirl wrapper')
    ...
    if args.algo == 'her':
        ...
        assert algo_ in {'td3', 'aimtd3', 'ddltd3', 'gailtd3'}, "{} is not compatible with HER".format(algo_)
    ...
    save_path = os.path.join(log_path, "{}_{}{}{}".format(env_id, args.seed, get_latest_run_id(log_path, "{}_{}".format(env_id, args.seed)) + 1, uuid_str))
    ...
    def create_env(n_envs, eval_env=False, no_log=False):
        ...
        print("Algo: {}, eval_env: {}".format(args.algo, eval_env))
        if args.algo == 'her' or args.algo == 'aimsac' or args.algo == 'sac':
            # Wrap the env if need to flatten the dict obs
            if isinstance(env, VecEnv):
                env = _UnvecWrapper(env)
            if args.smirl:
                env = SMiRLHERGoalEnvWrapper(env, eval_env)
            else:
    ...
    if algo_ in ['ddpg', 'sac', 'td3', 'aimtd3', 'ddltd3', 'gailtd3'] and hyperparams.get('noise_type') is not None:
