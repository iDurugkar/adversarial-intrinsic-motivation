...
from .replay_buffer import HindsightExperienceReplayWrapper, KEY_TO_GOAL_STRATEGY, DDLHindsightExperienceReplayWrapper
from .utils import HERGoalEnvWrapper, SMiRLHERGoalEnvWrapper
...
class HER(BaseRLModel):
    ...
    def __init__(self, policy, env, model_class, n_sampled_goal=4, use_ddl=False, use_gail=False,
                 use_smirl=False,
        ...
        self.expert_replay_wrapper = None
    ...
    def _create_replay_wrapper(self, env):
        ...
        """
        if not isinstance(env, (HERGoalEnvWrapper, SMiRLHERGoalEnvWrapper)):
            if self.use_smirl:
                env = SMiRLHERGoalEnvWrapper(env)
            else:
        ...
        wrapperClass = HindsightExperienceReplayWrapper if not self.use_ddl else DDLHindsightExperienceReplayWrapper

        self.replay_wrapper = functools.partial(wrapperClass,
                                                n_sampled_goal=self.n_sampled_goal,
                                                goal_selection_strategy=self.goal_selection_strategy,
                                                wrapped_env=self.env)
        # self.expert_replay_wrapper = functools.partial(wrapperClass,
        #                                                n_sampled_goal=self.n_sampled_goal,
        #                                                goal_selection_strategy=self.goal_selection_strategy,
        #                                                wrapped_env=self.env, expert=True)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="HER",
        ...
        return self.model.learn(total_timesteps, callback=callback, log_interval=log_interval,
                                ...
                                # expert_replay_wrapper=self.expert_replay_wrapper)
