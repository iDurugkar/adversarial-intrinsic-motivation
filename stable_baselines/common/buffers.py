...
class DDLReplayBuffer(object):
    ...
    def add(self, obs_t, action, reward, obs_tp1, done, dist):
        """
        add a new transition to the buffer
        ...
        :param done: (bool) is the episode done
        """
        data = (obs_t, action, reward, obs_tp1, done, dist)
    ...
    def extend(self, obs_t, action, reward, obs_tp1, done, dist):
        """
        add a new batch of transitions to the buffer
        ...
        Note: uses the same names as .add to keep compatibility with named argument passing
                but expects iterables and arrays with more than 1 dimensions
        """
        for data in zip(obs_t, action, reward, obs_tp1, done, dist):
            if self._next_idx >= len(self._storage):
                self._storage.append(data)
            else:
                self._storage[self._next_idx] = data
    ...
    def _encode_sample(self, idxes: Union[List[int], np.ndarray], env: Optional[VecNormalize] = None):
        obses_t, actions, rewards, obses_tp1, dones, dists = [], [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
            obs_t, action, reward, obs_tp1, done, dist = data
            obses_t.append(np.array(obs_t, copy=False))
            actions.append(np.array(action, copy=False))
            rewards.append(reward)
            obses_tp1.append(np.array(obs_tp1, copy=False))
