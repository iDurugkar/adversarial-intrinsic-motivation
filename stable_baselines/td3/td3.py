...
class TD3(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 policy_learning_delay=300, dense=False,  # ToDo: Please remember to turn this to False
                 target_policy_noise=0.2, rnd=False, target_noise_clip=0.5,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            self.rnd = RND(x_dim=self.observation_space.shape[0])
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    self.rnd_rew_ph = tf.placeholder(tf.float32, shape=(None, 1), name='rnd_rewards')
                    self.actions_ph = tf.placeholder(tf.float32, shape=(None,) + self.action_space.shape,
                                                     name='actions')
                    self.learning_rate_ph = tf.placeholder(tf.float32, [], name="learning_rate_ph")
                    rewards = self.rewards_ph + self.rnd_rew_ph
    ...
    def _train_step(self, step, writer, learning_rate, update_policy):
        ...
        #     # batch_rewards -= 1.
        if self.dense:
            goals = batch_next_obs[:, -self.goal_dim:]
            achieved_goals = batch_next_obs[:, -self.goal_dim * 2: - self.goal_dim]
            norm = np.square(achieved_goals - goals)
            norm = np.sqrt(np.sum(norm, axis=-1))
            self.max_r = max(np.max(norm), self.max_r)
            batch_rewards = - norm / self.max_r
        if self.use_rnd:
            rnd_rew = self.rnd.reward(batch_next_obs)
            rnd_loss = self.rnd.update(batch_next_obs)
            # batch_rewards += rnd_rew * 0.1
        else:
            rnd_rew = np.zeros_like(batch_rewards)
        ...
        feed_dict = {
            ...
            self.rnd_rew_ph: rnd_rew.reshape(self.batch_size, -1),
