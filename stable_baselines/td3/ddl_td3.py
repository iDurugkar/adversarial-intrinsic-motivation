...
from stable_baselines.common.buffers import DDLReplayBuffer
from stable_baselines.td3.policies import TD3Policy
from stable_baselines.td3 import RND
from stable_baselines.her.utils import HERGoalEnvWrapper
from stable_baselines.td3.dist_predictor import Predictor


class DDLTD3(OffPolicyRLModel):
    """
    ...
    :param target_policy_noise: (float) Standard deviation of Gaussian noise added to target policy
        (smoothing noise)
    :param rnd: (boolean) use RND exploration bonus
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 policy_learning_delay=300, dense=True,  # ToDo: Please remember to turn this to False
                 target_policy_noise=0.2, target_noise_clip=0.5,
                 random_exploration=0.0, verbose=0, tensorboard_log=None,
                 _init_setup_model=True, policy_kwargs=None,
                 full_tensorboard_log=False, seed=None, n_cpu_tf_sess=None):

        super(DDLTD3, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.predictor = Predictor(x_dim=self.goal_dim * 2)
                self.sess = tf_util.make_session(num_cpu=self.n_cpu_tf_sess, graph=self.graph)

                self.replay_buffer = DDLReplayBuffer(self.buffer_size)
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    action_loss = tf.reduce_mean(tf.square(3. * tf.maximum(tf.abs(policy_out) - 0.8, 0.)))  # 20.
                    ...
                    # policy_gvs = policy_optimizer.compute_gradients(policy_loss, var_list=tf_util.get_trainable_vars('model/pi'))
                    # capped_gvs = [(tf.clip_by_norm(grad, .01), var) for grad, var in policy_gvs]
                    # policy_train_op = policy_optimizer.apply_gradients(capped_gvs)
                    ...
                    tf.summary.scalar('learning_reward', tf.reduce_mean(self.rewards_ph))
    ...
    def _train_step(self, step, writer, learning_rate, update_policy):
        ...
        batch = self.replay_buffer.sample(self.batch_size, env=None)
        batch_obs, batch_actions, batch_rewards, batch_next_obs, batch_dones, batch_dists = batch

        disc_obs = batch_next_obs[:, -self.goal_dim * 2:]
        r1 = self.predictor.reward(disc_obs)
        batch_rewards += np.squeeze(r1)

        if self._vec_normalize_env is not None:
            batch_obs = self._vec_normalize_env.normalize_obs(np.array(batch_obs))
            batch_next_obs = self._vec_normalize_env.normalize_obs(np.array(batch_next_obs))
    ...
    def _update_discriminator(self):
        batch = self.replay_buffer.sample(self.batch_size, env=None)
        batch_obs, batch_actions, batch_rewards, batch_next_obs, batch_dones, batch_dists = batch
        batch_masks = np.where(batch_dists == -1., np.zeros_like(batch_dists), np.ones_like(batch_dists))
        disc_obs = batch_next_obs[:, -self.goal_dim * 2:]
        unequal_masks = np.where(np.prod(disc_obs[:, :self.goal_dim] == disc_obs[:, -self.goal_dim:],
                                       axis=-1, keepdims=True),
                                       np.zeros_like(batch_dists), np.ones_like(batch_dists))
        batch_masks *= unequal_masks
        disc_obs_2 = np.copy(disc_obs)
        disc_obs_2[:, :self.goal_dim] = disc_obs[:, -self.goal_dim:]
        disc_obs_2[:, -self.goal_dim:] = disc_obs[:, :self.goal_dim]
        disc_obs = np.concatenate([disc_obs, disc_obs_2], axis=0)
        batch_dists = np.concatenate([batch_dists, batch_dists], axis=0)
        batch_masks = np.concatenate([batch_masks, batch_masks], axis=0)
        pred_loss = self.predictor.update(disc_obs, batch_dists, batch_masks)
