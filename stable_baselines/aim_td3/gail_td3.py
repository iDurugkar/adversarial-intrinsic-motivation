...
class GAILTD3(OffPolicyRLModel):
    """
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 policy_learning_delay=0, reward='gail',
        ...
        super(GAILTD3, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.disc_replay_buffer = ReplayBuffer(100000)  # self.buffer_size // 300)
    ...
    def _train_discriminator(self):
        all_rewards = []
        target_batch = self.disc_replay_buffer.sample(self.disc_batch_size, env=self._vec_normalize_env)
        _, _, _, target_states, _ = target_batch
        target_states = target_states[:, -self.goal_dim * 2:]
        batch = self.replay_buffer.sample(self.disc_batch_size, env=self._vec_normalize_env)
    ...
    def learn(self, total_timesteps, callback=None,
        ...
        if expert_replay_wrapper is not None:
            self.disc_replay_buffer = expert_replay_wrapper(self.disc_replay_buffer)
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                self.disc_replay_buffer.add(obs_, action, reward_, new_obs_, done, info)
                obs = new_obs
